...
export function PaymentPanel({ expense, payment, onClose }: Props) {
  ...
  useEffect(() => {
    if (payment) {
      ...
      reset({
        ...
        wallet_currency_id: isPlanner
          ? payment.wallet_currency_code
          : (payment.user_currency_id ?? ''),
        notes: payment.notes ?? '',
      })
  ...
  }, [wallets, isEdit, isPlanner, setValue])

  const selectedCurrencyId = useWatch({ control, name: 'wallet_currency_id' })

  // For planners: the form value IS the currency code; for couples: look up from wallet list
  const selectedWallet = !isPlanner
    ? (wallets as UserCurrency[]).find(w => w.id === selectedCurrencyId)
    : null
  const walletCode = isPlanner
    ? selectedCurrencyId || baseCurrency
    : (selectedWallet?.currency_code ?? baseCurrency)
  ...
  }, [selectedCurrencyId, baseCurrency, isForeign, walletCode])

  const baseEquivalent =
    isForeign && walletAmount && exchangeRate
  ...
  function onSubmit(data: FormData) {
    ...
    if (isEdit && payment) {
      ...
      updatePayment.mutate(updatePayload as Parameters<typeof updatePayment.mutate>[0], {
        onSuccess: onClose,
      })
    } else {
      ...
      createPayment.mutate(createPayload as Parameters<typeof createPayment.mutate>[0], {
        onSuccess: onClose,
      })
    }
  }
        ...
        {isForeign ? (
          ...
          >
            ...
            <p style={{ fontSize: 12, color: '#B87820', margin: '0 0 12px' }}>
              Enter the exact rate from your app at the time of payment — it's locked to this record
              forever.
            </p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                ...
                {isRateFetching ? (
                  <div
                    ...
                    style={{ height: 34, borderRadius: 6, background: '#EDD9A3' }}
                  />
                ) : (
                  <input
                    type="number"
                ...
                <span style={{ fontSize: 10, color: '#9B9890' }}>
                  {isRateFetching
                    ? 'Fetching live rate…'
                    : 'Live rate pre-filled — edit to your exact app rate'}
                </span>
              </div>
